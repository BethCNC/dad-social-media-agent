...
- Interval: 30 seconds
- Timeout: 10 seconds
- Start Period: 40 seconds (increased for database initialization)
- Retries: 3

**Build Arguments:** (Important - Set before building!)
```bash
VITE_API_BASE_URL=https://your-domain.com
```
**Note:** This must match your production domain. It's baked into the frontend build and cannot be changed without rebuilding.
...
# Audio Mode Options:
# - MUTED_FOR_PLATFORM_MUSIC (RECOMMENDED - no background music, users add in platform)
# - AUTO_STOCK_ONLY (requires CC0 audio verification)
# - AUTO_STOCK_WITH_TIKTOK_HINTS (requires CC0 audio verification)
AUDIO_MODE=MUTED_FOR_PLATFORM_MUSIC
...
**Current Setup:** SQLite database and image uploads are stored locally in the container. **Without volumes, all data will be lost on redeployment!**

### **Required Persistent Volumes:**

#### 1. Database Volume (REQUIRED)
   - **Container Path:** `/app/backend`
   - **Purpose:** Preserves SQLite database across deployments
   - **Contains:** `content.db`, content bank items, schedules, user videos

#### 2. Image Uploads Volume (REQUIRED)
   - **Container Path:** `/app/static/uploads`
   - **Purpose:** Preserves AI-generated images and rendered videos
   - **Contains:** All generated visual assets
...
3. Add both volumes:
   ...
   **Volume 1:**
   - Name: `backend-data`
   - Source: Auto-generated by Coolify
...
After deployment, check that volumes are mounted:
```bash
# In Coolify terminal
ls -la /app/backend        # Should see content.db
ls -la /app/static/uploads  # Should see generated images
```
...
| `AUDIO_MODE` | No | `MUTED_FOR_PLATFORM_MUSIC` | Audio handling mode (MUTED recommended) |
